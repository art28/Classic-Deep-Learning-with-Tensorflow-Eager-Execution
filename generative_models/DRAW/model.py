...
class DRAW(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.read_attention_params = tf.layers.Dense(5)

        # Encoder layers
        self.lstm_encode = tf.nn.rnn_cell.BasicLSTMCell(256)
        self.encode_mu = tf.layers.Dense(z_dim)
        self.encode_logsigma = tf.layers.Dense(z_dim)

        # Decoder layers
        self.lstm_decode = tf.nn.rnn_cell.BasicLSTMCell(256)

        # Writing attention network
        self.write_attention_params = tf.layers.Dense(5)
        self.write_attention_dense = tf.layers.Dense(self.write_N * self.write_N)
        ...
        with tf.device(self.device_name):
            for i in range(epochs):
                ...
                for (X,) in ds:
                    ...
                    # clipping gradient(you can comment it if you want)
